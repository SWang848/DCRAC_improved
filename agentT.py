...
class DCRACAgent:
    ...
    def build_models(self):
        ...
        self.actor = Actor(self.nb_action, self.im_shape, self.nb_objective, self.qvalue_dim,
            ...
            weight_embd=self.feature_embd, backendtype=self.gpu_setting, tau=self.tau, clipnorm=self.clipnorm, clipvalue=self.clipvalue)
    ...
    def perform_updates(self, steps):
        ...
        if steps> self.min_buf_size * self.learning_steps:
            ...
            if steps % self.update_interval == 0:
                ...
                if steps % self.update_target_interval == 0:
...
class CNAgent(DCRACAgent):
    ...
    def pick_action(self, trace_o, trace_a, weights=None, egreedy=True):
        ...
        # q_value = self.critic.model.predict([weights[np.newaxis, ], trace_o[np.newaxis, ]])
